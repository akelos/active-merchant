<?php
ActiveMerchant::import('requires_parameter', 'country_code');
class ActiveMerchantCountry
{
    public $name;
    public $codes = array();
    /**
    ...
    public function __construct($options = null)
    {
        ...
        ActiveMerchantRequiresParameter::requires($options, array('name', 'alpha2', 'alpha3', 'numeric'));
    ...
    public function setAttributes(Array $attributes = array())
    {
        ...
        foreach($attributes as $v) {
            $this->codes[] = new ActiveMerchantCountryCode($v);
        }
    }
    /**
     * Checks if the given format is valid ??
     *
     * @param string $format
     */
    public function code($format)
    {
        return $this->_selectCountryCode($this->codes, $format);
    }
    private function _selectCountryCode(Array $array = array(), $format)
    {
        $ret = array();
    ...
    /**
     * Tries to find the matching country (could be iso code or country name)
     *
     * @param string $name
     * @return Country
     */
    ...
    {
        if(empty($name)) {
            throw new ActiveMerchantInvalidCountryCodeException('Cannot lookup country for an empty name');
        }
        $l = strlen($name);
        if($l == 2 || $l == 3) { // will try to find against iso code
            $upcase_name = strtoupper($name);
            $country_code = new ActiveMerchantCountryCode($name);
            $country = self::_detect(self::$countries, $country_code->getFormat(), $upcase_name);
        } else { // will try to find about the country name
            $country = self::_detect(self::$countries, 'name', $name);
        }
        if(!$country) {
            throw new ActiveMerchantInvalidCountryCodeException('No country could be found for the country ' . $name);
        }
        return new ActiveMerchantCountry($country);
    }
    /**
     * Walks the given array of arrays and looks for the first matching $key / $value
     *
     * @param array $array
     * @param string $key
     * @param string $value
     ...
     */
    private static function _detect(Array $array, $key, $value)
    {
...
class ActiveMerchantInvalidCountryCodeException extends Exception {}
